...
if (form) {
  ...
  form.addEventListener('submit', (event) => {
    ...
    if (!pickup || !drop || distance <= 0 || duration <= 0) {
      if (output) {
        output.textContent = 'Enter pickup, destination, and realistic trip details.';
      }
      return;
    }
    ...
    const surge = distance > 15 && duration > 35 ? 1.1 : 1;
    const totalFare = Math.round(fare * surge);

    if (output) {
      const surgeText = surge > 1 ? ' (includes peak hour adjustment)' : '';
